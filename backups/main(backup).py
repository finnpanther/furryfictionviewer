...
class FinalButton(Button):
    ...
    def on_press(self):
        ...
        app.root.display_text = condict[key]["string"]
...
class FunkyButton(Button):
    ...
    #The global key now updates to each unique button press
    def on_press(self):
        global key
        key = self.button_key
        app= App.get_running_app()
        app.root.display_text = app.root.display_text + "\n" + condict[key]["string"]
        app.root.generate_buttons()
...
class SceneScreen(FloatLayout):
    ...
    display_text = StringProperty(condict[key]["string"])
    button_text = StringProperty('The first button text')
    ...
    def generate_buttons(self):
        ...
        # Generate new buttons, each option from the key "points"
        if type(condict[key]["points"]) == list:
            for item in condict[key]["points"]:
                self.ids.player_responses.add_widget(FunkyButton(
                    text=condict[item]["string"], 
                    button_key=condict[item]["points"][0]
                    ))

        #The following line of code handles responses pointing to responses
        elif not condict[key]["points"].endswith("()"):
            newkey = condict[key]["points"]
            key = newkey
            app= App.get_running_app()
            app.root.display_text = app.root.display_text + condict[key]["string"]
            app.root.generate_buttons()
